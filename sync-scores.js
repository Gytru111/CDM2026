...
// M73-M88 (32es de finale) : scores saisis manuellement par l'admin.
// Le script ne doit ni lire ni écrire ces scores — ni ESPN, ni le backup.
...
const BRACKET_SF = [
  {id:101, src:[{id:97,type:'w'},{id:98,type:'w'}]},
  {id:102, src:[{id:99,type:'w'},{id:100,type:'w'}]},
];
...
async function fetchESPN(koArr) {
  ...
  for (const e of events) {
    ...
    if (found) {
      // Ignorer les 32es de finale (M73-M88) — scores saisis manuellement
      if (R32_IDS.has(found[0])) {
        continue;
      }
...
async function fetchWC26(koArr) {
  try {
    ...
    for (const g of games) {
      ...
      if (found) {
        // Ignorer les 32es de finale (M73-M88) — scores saisis manuellement
        if (R32_IDS.has(found[0])) continue;
